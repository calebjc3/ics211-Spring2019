...
public class HexadecimalSudoku {
  ...
  public static String numOfRecursionCalls;
  public static String numOfIterations;

  /**
   * Find an assignment of values to sudoku cells that makes the sudoku valid.
   * Implemented by Caleb Cheshire. March 15, 2019
   *        Found help for some aspects of implementing the solveSudoku method at https://github.com/soeux & https://stackoverflow.com/questions/18191648/sudoku-recursion-with-backtracking
   *        Worked on some of the concepts with Justin Chen.
  ...
  public static boolean solveSudoku(int[][] sudoku) {
    // Looks for the first empty cell.
  ...
  public static boolean checkSudoku(int[][] sudoku, boolean printErrors) {
    ...
    /* checks each cell for conflicts */
    for (int i = 0; i < sudoku.length; i++) {
      for (int j = 0; j < sudoku.length; j++) {
        int cell = sudoku[i][j];
        if (cell == -1) {
        ...
        /* checks if it matches another value in the same column. */
        ...
        /* checks if it matches another value in the 4x4 grid. */
        for (int k = 0; k < 4; k++) {
          for (int m = 0; m < 4; m++) {
